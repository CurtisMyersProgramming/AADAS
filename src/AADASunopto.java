...
public class AADASunopto {
	...
	public static void flightPhase(List<AADAS> crash) {
		...
		for (int i = 0; i < crash.size(); i++) { // reading through inital array list through whole csv
			AADAS currentCrash = crash.get(i); // populating array with lines
			phases.add(currentCrash.BroadPhaseofFlight); // adding EventDate to years arraylist characted 6 - 10 (year 4
															// digits)
		...
		System.out.println("\nThere are " + phaseslisted.size() + " phases of flight stages available"); // prints how
																											// many
																											// years of
																											// data are
																											// shown
	...
	public static void Feature4(List<AADAS> crash) throws FileNotFoundException {
		...
		while (!s.equals("Q")) {
			...
			System.out.println(
					"2 - Flights that were deadly but the amount of survivors was greater than or equal to the amount perished");
			...
			if (selection.equals("1")) {
				...
				System.out.println("\nThere are " + p + " records that match the criteria of eveybody onboard dying"); // show
																														// how
																														// many
																														// records
																														// match
	...
	public static void Feature5phase(List<AADAS> crash) throws FileNotFoundException {
		...
		outputCrashes(matches); // call method to output all crashes with the array matches for what to print
		System.out.println("\nThere are " + matches.size() + " records that contain " + inputflight); // print out
																										// message
																										// showing user
																										// how many
																										// records match
	...
	public static void Feature5years(List<AADAS> crash) {
		System.out.println(
				"This option will show you all records for the option previously selected in a 10 year window of your choice.");// output
																																// message
																																// to
																																// user
		...
		for (AADAS c : crash) { // each loop of eventdate (2001, 2002, 2003....) run the whole array
			...
			if (year >= chosenYear && year <= tenyear) { // get year parse uses the get year method and parses it to an
															// int for comparison reasons
															// get year parse will check for each loop if i has any
															// matches if it does add those to the array
				matches.add(c); // add to array
			}

			// for(int i = chosenYear; i <= tenyear; i++) { // i = event date and iterate
			// eventdate until it is equal to ten year
			// for(AADAS c : crash) { // each loop of eventdate (2001, 2002, 2003....) run
			// the whole array
			// if (c.getYearparse() == i) { // get year parse uses the get year method and
			// parses it to an int for comparison reasons
			// // get year parse will check for each loop if i has any matches if it does
			...
			System.out.println("\nThere are " + matches.size()
					+ " records which match that criteria in this 10 year period between " + choice + " and "
					+ tenyear); // output letting user know how records match
		}
	}
	...
	public static void Feature6(List<AADAS> crash) throws FileNotFoundException {
		...
		year(crash);
		System.out.println(
				"Please specify the year that you would like to view 10 years worth of data from : e.g. 2001, 1978\n");
		...
		for (int i = Eventyear; i <= tenyear; i++) { // i = event date and iterate eventdate until it is equal to ten
														// year
			for (AADAS c : crash) { // each loop of eventdate (2001, 2002, 2003....) run the whole array
				if (c.getYearparse() == i) { // get year parse uses the get year method and parses it to an int for
												// comparison reasons
												// get year parse will check for each loop if i has any matches if it
												// does add those to the array
					matches.add(c); // add to array
				}
			}
		}
		// outputCrashes(matches); // output all crashes in array
		System.out.println(
				"\nThere are " + matches.size() + " records which match that criteria in this 10 year period between "
						+ choice + " and " + tenyear + "\n"); // output letting user know how records match

		// Creating a String ArrayList to store the Make and Model and Integer Arrays to
		// count variables
...
//find appropriate MakeRecord (or create a new one)
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			int index = makeModel.indexOf(makeModelStr);
			int acccount = makeModelAccidentCount.get(index);
			
			
			makeModelAccidentCount.set(index, acccount);

			amount.add(new MakeRecord(makeModelStr, acccount));
		...
		// loop through the makeModel array and output all makes/models with their
		// accident rate, and highI determines the highest accident rate
		...
		System.out.println(
				"[ 3 - View the Highest Fatality Count in Reverse between " + choice + " and " + tenyear + "]");
		System.out.println(
				"[ 4 - View the Highest Accident Rates in Reverse between " + choice + " and " + tenyear + "]");
